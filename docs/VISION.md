...
- The **switchboard is the centerpiece** of the flagship "dispatcher" workflow —
  ...
  - **Self-contained.** Cars, players, and locomotives are **not** shown in the
    ...
    occupancy, and paths — not rolling stock or people.
...
1. The one in-game junction that breaks a station's mapping on **both** the
   single-track and DoubleTrack layouts. (Owner: maintainer.)
2. **Block colouring** — the meaning the colours are *supposed* to convey does
   not yet match what the code renders. Get the semantics right.
3. **Disabling pathing** must be a clean, supported operation (currently not).
4. **Map ⇄ switchboard view switching** must not break active pathing (it
   currently does).
5. **Path conflicts** between trains: no complete live-resolution exists yet.
...
4. Grow around the switchboard as a standalone, opt-in dispatcher surface,
   without pulling cars/players/locos into it.
...
- Representing cars / players / locomotives inside the switchboard.
